...
def normalize_siena_channels(raw):
    rename_map = {
        ch: EDF_CHANNEL_RENAMES[ch]
        for ch in raw.info["ch_names"]
        if ch in EDF_CHANNEL_RENAMES
    }
    ...
    for gk, ginfo in tqdm(
        ...
        for fname, finfo in tqdm(
            ...
            raw = mne.io.read_raw_edf(
                edf_path,
                include=EDF_CHANNELS,
                preload=True,
                verbose="ERROR",
            )

            # Siena EDF channel names have an EEG prefix and a few capitalization variants.
            normalize_siena_channels(raw)
...
def main():
    ...
    df = pd.read_csv(SUBJECT_INFO_FILE, skipinitialspace=True)
    subj_info = {
        row["patient_id"]: {"gender": row["gender"], "age": float(row["age_years"])}
